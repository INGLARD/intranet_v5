        ...
        'female' => array(
            ...
            */
            private function build_nav_array($nav_array) {
                ...
                $gg=0;
                $i1=0;
                $this->nav_html .='<div class="submenus_trigger2 submenu_background" style="display:none;"></div>';
                $this->nav_html .='<div class="container_menu">';
                    foreach ($nav_array as $node) {
                        // Get all vital link info
                        $i=1;

                        $ia=$ia+count($node['name']);
                        $ia2=$ia+count($node['name'])/4;

                        //echo $ia."<br>";


                        $link_name      = isset($node['name']) ? $node['name'] : '';
                        $link_subname   = isset($node['subname']) ? $node['subname'] : '';
                        $link_icon      = isset($node['icon']) ?  $node['icon']  : '';
                        $link_url       = isset($node['url']) ? $node['url'] : '#';
                        $link_code      = isset($node['code']) ? $node['code'] : '';
                        ...
                        if ($link_code) {
                            ...
                            $this->nav_html .= "<div class='Trigger' id='menu".$i1."'>\n";


                                //             $this->nav_html .= '<div class="col-sm"><div class=" btmenu '.strtolower($link_name).'"><img src="assets/img/img_menu/'.$link_icon.'"></div><h1 class="titreicop">'.$link_name.'</h1></div>
                                // <div class="ff caret" ></div>';

                                $this->nav_html .= '<div class="main_icon"><div class=" btmenu '.strtolower($link_subname).'"  name="main01" data-subname="'.$link_subname.'" data-code="'.$link_code .'"  data-title="'.$link_name .'"><img src="assets/img/img_menu/'.$link_icon.'">
                                </div><h1 class="titreicon">'.$link_name.'</h1></div>';
